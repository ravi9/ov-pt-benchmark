...
def benchmark_pt(image, ts_mode=False):
    model = resnet50()
    ...
    print(f'\nBenchmarking PyTorch Inference w/ TorchScript={ts_mode} for {num_iterations} iterations')
    inf_time_arr = []
    ...
    print(f'PyTorch Average inference time  w/ TorchScript={ts_mode} with f32: {np.mean(inf_time_arr):.2f} seconds')
...
if __name__ == '__main__':
  ...
  parser.add_argument('-ni', '--num_iterations', type=int, default=500)
  ...
  ov_fp32_times = benchmark_ov(image, "f32")
  ov_bf16_times = benchmark_ov(image, "bf16")

  total_time = [np.sum(pt_fp32_times), np.sum(pt_ts_fp32_times), np.sum(ov_fp32_times), np.sum(ov_bf16_times)]
  median_time = [np.median(pt_fp32_times), np.median(pt_ts_fp32_times), np.median(ov_fp32_times), np.median(ov_bf16_times)]
  average_time = [np.mean(pt_fp32_times), np.mean(pt_ts_fp32_times), np.mean(ov_fp32_times), np.mean(ov_bf16_times)]
  std_dev = [np.std(pt_fp32_times), np.std(pt_ts_fp32_times), np.std(ov_fp32_times), np.std(ov_bf16_times)]
  min_time = [np.min(pt_fp32_times), np.min(pt_ts_fp32_times), np.min(ov_fp32_times), np.min(ov_bf16_times)]
  max_time = [np.max(pt_fp32_times), np.max(pt_ts_fp32_times), np.max(ov_fp32_times), np.max(ov_bf16_times)]
  fps = [ 1 / np.mean(pt_fp32_times), 1 / np.mean(pt_ts_fp32_times), 1 / np.mean(ov_fp32_times), 1 / np.mean(ov_bf16_times)]

  print(f"\nImage size: {image.shape}")
  print("\nmetric,pt_fp32,pt_ts_fp32,ov_fp32,ov_bf16")
  print("total_time,{:.2f},{:.2f},{:.2f},{:.2f}".format(*total_time))
  print("median,{:.2f},{:.2f},{:.2f},{:.2f}".format(*median_time))
  print("average,{:.2f},{:.2f},{:.2f},{:.2f}".format(*average_time))
  print("std_dev, {:.2f}, {:.2f}, {:.2f}, {:.2f}".format(*std_dev))
  print("min,{:.2f},{:.2f},{:.2f},{:.2f}".format(*min_time))
  print("max,{:.2f},{:.2f},{:.2f},{:.2f}".format(*max_time))
  print("fps,{:.2f},{:.2f},{:.2f},{:.2f}".format(*fps))
